...
const Dealers = () => {
  // Static dealer directory; update here when onboarding/removing a dealer
  const dealers = [
    {
      id: 1,
  ...
  const states = useMemo(
    () => ["All", ...Array.from(new Set(dealers.map((d) => d.state)))],
    [dealers]
  );
  ...
  const filteredDealers = useMemo(() => {
    ...
    return dealers.filter((dealer) => {
      const matchesState = activeState === "All" || dealer.state === activeState;
      const matchesQuery =
        !q ||
  ...
  }, [dealers, query, activeState]);

  const hasFilters = query.trim() !== "" || activeState !== "All";

  return (
    <div className="min-h-screen bg-bg dark:bg-d-bg">
      <SEO
        title="Our Dealers - Neoplatron"
        description="Meet Neoplatron's authorized dealers and distributors, and learn how to become one yourself."
        canonical="/partners/dealers"
        keywords="Neoplatron dealer, Neoplatron distributor, dealership opportunity, distribution partner"
      />
      ...
      <section className="relative bg-linear-to-br from-primary via-secondary to-primary text-white py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          ...
          <p className="text-sm sm:text-base lg:text-lg text-bg-light/90 max-w-2xl mx-auto leading-relaxed font-light mb-6 sm:mb-8">
            Connecting our innovative green mobility solutions with communities around the world through our authorized dealers and distributors.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/15 rounded-full px-4 py-2">
              ...
              <span className="text-sm font-medium">
                {dealers.length} Authorized Dealers
              </span>
            </div>
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/15 rounded-full px-4 py-2">
          ...
          {filteredDealers.length > 0 ? (
            ...
            <div className="text-center py-16 px-4 bg-bg-light dark:bg-d-bg-light rounded-2xl border border-dashed border-border dark:border-d-border">
              ...
              <p className="text-sm text-text-para dark:text-d-text-para max-w-md mx-auto mb-6">
                We couldn't find a dealer matching your search. Try a different city or state — or be the first to bring Neoplatron to your area.
              </p>
              <button
                type="button"
                onClick={() => {
